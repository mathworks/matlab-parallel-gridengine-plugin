...
if isempty(cluster.UserData)
    ...
    if ~isstruct(cluster.UserData)
        error('parallelexamples:GenericGridEngine:IncorrectUserData', ...
...
if useUI
    ...
    if isempty(usernameResponse)
        % User hit cancel
        error('parallelexamples:GenericGridEngine:UserCancelledOperation', ...
...
if useUI
    ...
    if strcmp(passphraseResponse, 'Cancel')
        % User hit cancel
        error('parallelexamples:GenericGridEngine:UserCancelledOperation', 'User cancelled operation.');
    end
