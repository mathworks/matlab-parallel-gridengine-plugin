...
This repository contains MATLAB code files and shell scripts that you can use to submit jobs from a MATLAB or Simulink session running on Windows&reg;, Linux&reg;, or macOS to a Grid Engine&reg; scheduler running on Linux.
...
This is typically true if your MATLAB session is running on the same Grid Engine cluster you want to submit to.
...
This submission mode is useful for submitting from a MATLAB session on a Windows computer to a Linux Grid Engine cluster on the same network.
Your Windows machine creates an SSH session to the cluster head node to access the Grid Engine utilities, while using a shared networked folder to store job data files.
...
```matlab
c.AdditionalProperties.ClusterHost = 'gridengine01.organization.com'; % MATLAB will SSH to gridengine01 to submit jobs
```
...
```matlab
saveAsProfile(c, "myGrid EngineCluster");
```
...
```matlab
c = parcluster("myGrid EngineCluster")
```
